...
pub fn image(this: @This()) seizer.image.Linear(seizer.color.argb8888) {
    return .{
        .size = this.size,
        .stride = this.size[0],
        .pixels = this.pixels,
...
pub fn canvas_blit(this_opaque: ?*anyopaque, pos: [2]f64, src_image: seizer.image.Linear(seizer.color.argb8888)) void {
    const this: *@This() = @ptrCast(@alignCast(this_opaque));
    const pos_i = [2]i32{
...
pub fn canvas_textureRect(this_opaque: ?*anyopaque, dst_pos: [2]f64, dst_size: [2]f64, src_image: seizer.image.Linear(seizer.color.argb8888), options: seizer.Canvas.RectOptions) void {
    const this: *@This() = @ptrCast(@alignCast(this_opaque));
