...
pages: std.AutoHashMapUnmanaged(u32, seizer.image.Image(seizer.color.argb8888)),
glyphs: GlyphMap,
line_height: f64,
base: f64,
scale: [2]f64,
...
pub fn fromFileContents(allocator: std.mem.Allocator, font_contents: []const u8, image_list: []const ImageFile) !@This() {
    ...
    var pages = std.AutoHashMapUnmanaged(u32, seizer.image.Image(seizer.color.argb8888)){};
    ...
    while (page_name_iterator.next()) |entry| {
        ...
        var image = try seizer.image.Image(seizer.color.argb8888).fromMemory(allocator, image_content);
