...
var bicubic_upscale: seizer.image.Image(seizer.color.argbf32) = undefined;
var bicubic_downscale: seizer.image.Image(seizer.color.argbf32) = undefined;
...
pub fn init() !void {
    ...
    bicubic_upscale = try seizer.image.Image(seizer.color.argbf32).alloc(gpa.allocator(), .{ 5 * image.size[0], 5 * image.size[1] });
    errdefer bicubic_upscale.free(gpa.allocator());
    bicubic_upscale.resize(image);

    bicubic_downscale = try seizer.image.Image(seizer.color.argbf32).alloc(gpa.allocator(), .{ image.size[0] / 2, image.size[1] / 2 });
