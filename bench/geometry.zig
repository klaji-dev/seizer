...
pub fn main() !void {
    ...
    for (&boxes_f32, &boxes_f64, &boxes_i32, &boxes_i64) |*bf32, *bf64, *bi32, *bi64| {
        ...
        bi32.*[0] = @intFromFloat(bf64.*[0] * (std.math.maxInt(i32) >> 1));
        bi32.*[1] = @intFromFloat(bf64.*[1] * (std.math.maxInt(i32) >> 1));
        bi32.*[2] = @intFromFloat(bf64.*[2] * (std.math.maxInt(i32) >> 1));
        bi32.*[3] = @intFromFloat(bf64.*[3] * (std.math.maxInt(i32) >> 1));

        bi64.*[0] = @intFromFloat(bf64.*[0] * (std.math.maxInt(i64) >> 1));
        bi64.*[1] = @intFromFloat(bf64.*[1] * (std.math.maxInt(i64) >> 1));
        bi64.*[2] = @intFromFloat(bf64.*[2] * (std.math.maxInt(i64) >> 1));
        bi64.*[3] = @intFromFloat(bf64.*[3] * (std.math.maxInt(i64) >> 1));
    }
